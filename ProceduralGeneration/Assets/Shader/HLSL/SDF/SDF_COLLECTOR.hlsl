...
//Thanks to Signed Distance Field 2D : https://iquilezles.org/articles/distfunctions2d/

float dot2( in float2 v ) { return dot(v,v); }
float ndot(float2 a, float2 b ) { return a.x*b.x - a.y*b.y; }
float mod(float x, float y) {return x - y * floor(x/y); }

void SDFCircle_float(float2 p, float1 r, out float1 OutCircle)
{
    OutCircle = length(p) - r;
}

void SDFRoundedBox_float(float2 p, float2 scale, float4 roundexCorner , out float1 OutRoundedBox)
{
...
void SDFOrientedBox_float(float2 p, float2 a, float2 b ,float1 th,out float1 OUTOrientedBox)
{
    float l = length(b-a);
    float2  d = (b-a)/l;
    float2  q = (p-(a+b)*0.5);
...
void SDFSegment_float(float2 p, float2 a, float2 b, out float1 OutSegment)
{
...
void SDFRhombus_float(float2 p, float2 b , out float1 OutRhombus) 
{
    p = abs(p); 
...
void SDFTrapezoid_float(float2 p, float1 r1, float1 r2, float1 he, out float1 OutTrapezoid )
{
...
void SDFParallelogram_float(float2 p, float1 wi, float1 he, float1 sk, out float1 OutParallelogram )
{
...
void SDFEquilateralTriangle_float(float2 p, float1 scale , out float1 OutEquilateralTriangle)
{
...
void SDFTriangleIsosceles_float( float2 p, float2 q, float1 scale , out float1 OutIsoceleTriangle)
{
    p.x = abs(p.x);
...
void SDFTriangle_float(float2 p, float2 p0, float2 p1, float2 p2, float1 scale , out float1 OutTriangle)
{
    p *= scale;
...
void SDFUnevenCapsule_float(float2 p, float1 r1, float1 r2, float1 h , float1 scale,out float1 UnevenCapsule )
{
    p *= scale;
    p.x = abs(p.x);
    float b = (r1-r2)/h;
...
void SDFHexagon_float(float2 p, float1 r, out float1 OutHexagon )
{
...
void SDFOctogon_float(float2 p, float1 r , out float1 OutOctogon)
{
...
void SDFHexagram_float(float2 p, float1 r, out float1 OutHexagram )
{
...
void SDFStar5_float(float2 p,  float1 r,  float1 rf, out float1 OutStar5 )
{
...
// A CORRIGER
void SDFStar_float(float2 p,float1 r,float1 n, float1 m, out float1 OutSDFStar)
{
...
void SDFPie_float( float2 p, float2 c, float1 r, out float1 OutSDFPie)
{
    p.x = abs(p.x);
    float l = length(p) - r;
...
void SDFCutDisk_float(float2 p, float1 r, float1 h, out float1 OutSDFCutDisk )
{
...
void SDFArc_float(float2 p, float2 sc,  float1 ra, float1 rb, out float1 OutSDFArc )
{
...
void SDFEgg_float(float2 p, float2 ra, float1 rb, out float1 OutEgg)
{
    const float k = sqrt(3.0);
    p.x = abs(p.x);
    float r = ra - rb;
...
void SDFHeart_float(float2 p, float1 scale, out float1 OutSDFHeart )
{
    p /=10;
    p.x = abs(p.x);
    if( p.y+p.x>1.0 )
...
void SDFStairs_float(float2 p, float2 wh, float1 n ,out float1 OutStairs)
{
    float2 ba = wh*n;
