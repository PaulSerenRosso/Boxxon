...
public class BowyerWatson
{
    ...
    public BowyerWatson(Rect _rect, float _superTriangleBaseEdgeOffset, Vector2[] _points)
    {
        ...
        points = _points;
        superTriangle = MathHelper.GetTriangleWitchInscribesRect(rect, superTriangleBaseEdgeOffset);
    }
    ...
    public Triangle[] Triangulate()
    {
            ...
            for (int j = trianglesChoosen.Count-1 ; j >= 0 ;j--)
        {
            ...
            {
                Triangle newTriangle = new Triangle(points[i], polygone[j].PointA, polygone[j].PointB);
        ...
        {
            if (!superTriangle.TriangleHasSharedVertices(triangle.Key))
            {
