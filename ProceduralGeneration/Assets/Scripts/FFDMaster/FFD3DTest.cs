...
namespace FFDHelper
{
	public class FFD3DTest : MonoBehaviour
	{
				...
				for (y = 0.0f; y < 1.0f; y += 1.0f / CPs_t)
				{
					for (z = 0.0f; z < 1.0f; z += 1.0f / CPs_u, i++)
					{
						...
						Node.transform.localPosition =
							(p0 + new Vector3(x * S, y * T, z * U) *
								2); // position is min node + % across the object * scale
		...
		Vector3 ReconstructVertex(int index)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				for (int j = 0; j < 2; j++)
				{
					for (int k = 0; k < 2; k++)
					{
						point += controlPointsGO[k + (j * 2) + (i * 2 * 2)].transform.localPosition *
						         (Bs[i] * Bt[j] * Bu[k]);
					}
				}
			}
